...
string promptForString( const string &promptString )
   {
    ...
    // check for end of line in advance of input
    firstChar = cin.get();
    ...
    // check for first character not being endline character
     if( firstChar != LINEFEED_CHAR )
       {
        // prepend first character onto string
        response = firstChar + response;
       }
...
void promptForString( const string &promptString, char resultString[] )
   {
    ...
    tempString = promptForString( promptString );

    strcpy( resultString, tempString.c_str() );
   }
